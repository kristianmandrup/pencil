 ...
 * Inherits from Bike
 * 
 * @tanks https://github.com/gabrieleds/bike
 * @api public
 */
...
jade.filters.stylus = jade.filters.styl = function(str, options){
  ...
  if(options && options.id){
    return '<style type="text/css" id="' + options.id.replace(/'/g, '') + '">' + ret + '</style>';
  }
  else{
...
Pencil.render = function Render(file, options, just){
  options = options || {};
  ...
  var text = fs.readFileSync(options.filename, 'utf8')
    , fn = Pencil.compile(text, options)
    ;
    
  return (just ? fn : fn(options));
};
...
Pencil.parse = function(tag, parent){
  ...
  //tag.index = tag.index || 0;
  
  if(tag.isBlock){
    
    // Parse children
    _.map(tag.nodes, function(child, index){
      //child.state = state;
      child.index = index;
      var node = Pencil.parse(child, tag);
      //node.index = index;
    ...
    if(tag.block && tag.block.nodes){
      _.map(tag.block.nodes, function(child, index){
        //child.state = state;
        child.index = index;
        var node = Pencil.parse(child, tag);
        //node.index = index;
...
Pencil.template = function Template(tag, file, options){
  ...
  if(~Cache.defaults.indexOf(tagname)){  //if(_.include(Cache.defaults, tagname)){}
    ...
    if(!tag){
      ...
      try{
        ...
        console.warn('Pencil ERR: Custom tag "' + tagname + '" not found.')
        //throw new Error('Pencil ERR: Custom tag "' + tagname + '" not found.')
      };
    }
...
Pencil.register([
  ...
  path.join(__dirname, '../namespaces/ui'),
  path.join(__dirname, '../namespaces/angular'),
