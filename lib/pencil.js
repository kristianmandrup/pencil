...
var pencil = module.exports = utils.extend({}, jade);
...
pencil.inherits = function inherits (target, origin) {
  utils.inherits(target, 'string' === typeof origin
...
pencil.use = function namespace (directory) {
  for (var i = 0; i < arguments.length; i++) {
    if ('default' == arguments[i]) {
      cache.setNamespace(path.join(__dirname, '../namespaces/pencil'));
    }
    else if (utils.isPath(arguments[i])) {
      // namespace
      cache.setNamespace(arguments[i]);
    }
    else {
      // vendore
      cache.setNamespace(require(arguments[i]).namespace);
    }
  }
