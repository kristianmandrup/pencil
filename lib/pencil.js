...
pencil.Parser = jade.Parser = require('./pencil/parser');
...
.forEach(function (node) {
  cache.setTag(
    'pencil.' + node.toLowerCase(),
    pencil.nodes[node] = require('./pencil/nodes/' + node.toLowerCase())
  );
  /*
  var name = node.toLowerCase()
    , fetch = function () {
        delete cache.tags['pencil.' + name];
        delete pencil.nodes[node];

        return pencil.nodes[node] = cache.tags['pencil.' + name] = require('./pencil/nodes/' + name);
      }
    ;

  cache.tags.__defineGetter__('pencil.' + name, fetch);
  pencil.nodes.__defineGetter__(node, fetch);
  */
  ...
  Object.defineProperty(cache.tags, 'pencil.' + name, {
    get: fetch,
    configurable: true,
    enumerable: true
  });
  Object.defineProperty(pencil.nodes, node, {
    get: fetch,
    configurable: true,
    enumerable: true
  });
