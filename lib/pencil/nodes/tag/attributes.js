...
function slice (val, count) {
  return val ? Array.prototype.slice.call(val, count || 0) : [];
};
...
module.exports = {
  ...
  'getAttr': function (name) {
    ...
    if (1 === arguments.length) {
      return unescape(this.getAttribute(name));
    }
    else {
      var ret = {}
        , args = slice(arguments)
      ...
      for (var i = 0; i < this.attrs.length; i++) {
        if (~args.indexOf(this.attrs[i].name)) {
          ret[this.attrs[i].name] = unescape(this.attrs[i].val);
        }
      };
  ...
  'setAttr': function (name, value) {
    ...
    if (pencil.isCode(value)) {
      ...
      this.setAttribute(name, escape(value || ''), true);
    }
