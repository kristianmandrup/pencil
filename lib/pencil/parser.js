...
utils.inherits(Parser, jade.Parser);
  ...
  function reParse (node, parent, realParent, index) {
    ...
    if (pencil.isBlock(node)) {
      ...
      if (!pencil.isComponent(node)) {
        ...
        // HUMMM, check this. The problem is that it renders EVERY tag with a block
        // what if we have conditional tags ? they all would get rendered anyway
        for (var i = 0; i < node.block.nodes.length; i++) {
          if(node.block.nodes[i].hasChildren()) {
            reParse(node.block.nodes[i].block, node, node);
          }
        };
      }
    }
